        ...
        for(Map.Entry<String,Object> entry: ioc.entrySet()) {
            ...
            if(aClass.isAnnotationPresent(LagouRequestMapping.class)) {
                ...
                if (aClass.isAnnotationPresent(Security.class)){
                    Security authAnno= aClass.getAnnotation(Security.class);
                    String[] value = authAnno.value();
                    if (value!=null&&value.length!=0){
                        Set<String> ruleSet=new HashSet<>();
                        for (String s:value){
                            ruleSet.add(s);
                        }
                        SecurityManager.addAuthRule(Pattern.compile(baseUrl),ruleSet);
                    }
                }
            }
            ...
            for (int i = 0; i < methods.length; i++) {
                ...
                if (method.isAnnotationPresent(Security.class)){
                    Security authAnno= method.getAnnotation(Security.class);
                    String[] value = authAnno.value();
                    if (value!=null&&value.length!=0){
                        Set<String> ruleSet=new HashSet<>();
                        for (String s:value){
                        ...
                        SecurityManager.addAuthRule(Pattern.compile(url),ruleSet);
                    }
                }
...
//        method.invoke() //
        ...
        if (!SecurityManager.hasAuth(req)){
            resp.getWriter().write("has not arthority");
            return;
        }
